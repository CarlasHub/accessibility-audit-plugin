...
1. Confirm the exact pages/input and auditor in one concise interaction. Use the MCP form when available. Use `Automated` as the editable default. Do not ask for page details already present in a supplied list.
...
3. Run desktop, mobile, and 320px reflow headlessly with screenshots enabled. Surface MCP progress.
4. If the user stops the run, let the tool close Chromium and write and validate partial JSON/XLSX output. Report it as cancelled, not complete.
...
6. Keep findings on separate page rows by default. Merge only when evidence proves the same reusable component implementation and root cause; list each URL separately in Links.
7. Validate the workbook. Report run status, exact workbook/JSON paths, completed/partial/not-started page counts, finding counts, Image Inventory count, and validation result.
...
- Image Inventory contains unique finding screenshots with embedded previews; no screen-reader worksheet is created.
