...
6. Use one row for the same reusable component implementation and root cause across affected pages. Group repeated instances within that component instead of creating one row per DOM node. Keep page-specific implementations or distinct root causes separate, and list each affected URL separately in Links. Treat missing `aria-controls` alone as a Best Practice review for ordinary disclosures/accordions. Combine related unchanged-state evidence on the same component, but confirm a stale `aria-expanded` value only when controlled content is also observed to open; otherwise retain a review. Do not require generic disclosures/accordions to close with Escape.
