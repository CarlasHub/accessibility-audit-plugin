...
Missing `aria-controls` alone is not treated as a WCAG failure or a standalone review for an ordinary disclosure or accordion because that relationship is optional in those patterns. Disclosure automation first establishes a collapsed baseline, then tests Enter and Space and compares `aria-expanded` with controlled-content visibility. A state finding is confirmed only when the performed interaction produces an observed mismatch. A failed setup or ambiguous result remains raw JSON and tested-inconclusive coverage evidence; it does not create a workbook finding, failure, or pass. Generic disclosure automation does not require Escape to close the component.
