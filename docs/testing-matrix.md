...
| Disclosure state, focus order, and Escape | Whether expandable controls expose state/content and behave coherently from the keyboard | Yes | Yes | Yes | State failures confirmed; focus sequence and optional Escape behavior reviewed |
...
| Target size | Whether rendered controls are smaller than the minimum dimensions considered by the rule | Yes | Yes | Yes | Reviewed because spacing and other exceptions apply |
